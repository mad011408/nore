...
const useAuth = () => ({
  user: { id: "dev-user-001", email: "dev@hackerai.local" },
  entitlements: ["pro-plan"], // Give dev user pro access
  isLoading: false,
});
...
export const GlobalStateProvider: React.FC<GlobalStateProviderProps> = ({
  ...
  // Initialize chat mode with default value to match server render
  // Then sync with localStorage after hydration to prevent hydration mismatch
  const [chatMode, setChatModeState] = useState<ChatMode>("ask");
  ...
  // Sync chat mode from localStorage after hydration (client-side only)
  useEffect(() => {
    const saved = readChatMode();
    if (saved === "ask" || saved === "agent") {
      setChatModeState(saved);
    }
  }, []);

  // Initialize chat sidebar state - start with false to match server render
  // Then sync with localStorage after hydration to prevent hydration mismatch
  const [chatSidebarOpen, setChatSidebarOpen] = useState(false);
  const [hasMounted, setHasMounted] = useState(false);

  // Sync sidebar state from localStorage after initial render (client-side only)
  useEffect(() => {
    setHasMounted(true);
    if (!isMobile) {
      const savedState = chatSidebarStorage.get(isMobile);
      setChatSidebarOpen(savedState);
    }
  }, []);
  ...
  // Queue behavior preference - initialize with default to match server render
  const [queueBehavior, setQueueBehaviorState] = useState<QueueBehavior>("queue");

  // Sandbox preference - initialize with default to match server render
  const [sandboxPreference, setSandboxPreferenceState] =
    useState<SandboxPreference>("e2b");

  // Selected AI model - initialize with default to match server render
  const [selectedModel, setSelectedModelState] = useState<SelectedModel>("default");

  // Sync localStorage values after hydration to prevent hydration mismatch
  useEffect(() => {
    // Queue behavior
    const savedQueueBehavior = localStorage.getItem("queue-behavior");
    if (savedQueueBehavior === "queue" || savedQueueBehavior === "stop-and-send") {
      setQueueBehaviorState(savedQueueBehavior);
    }

    // Sandbox preference
    const savedSandbox = localStorage.getItem("sandbox-preference");
    if (savedSandbox) {
      setSandboxPreferenceState(savedSandbox);
    }
    ...
    if (
      savedModel === "default" ||
      savedModel === "claude-opus-4-5" ||
      savedModel === "gemini-3-pro" ||
      savedModel === "gpt-5.2-pro" ||
      savedModel === "gpt-5.2-thinking"
    ) {
  ...
  const setQueueBehavior = useCallback((behavior: QueueBehavior) => {
    setQueueBehaviorState(behavior);
    if (typeof window !== "undefined") {
      localStorage.setItem("queue-behavior", behavior);
    }
  }, []);
  ...
  // Initialize URL-based states with defaults to match server render
  // Then sync with URL params after hydration
  const [temporaryChatsEnabled, setTemporaryChatsEnabled] = useState(false);
  const [teamPricingDialogOpen, setTeamPricingDialogOpen] = useState(false);
  const [teamWelcomeDialogOpen, setTeamWelcomeDialogOpen] = useState(false);
  const [migrateFromPentestgptDialogOpen, setMigrateFromPentestgptDialogOpen] =
    useState(false);

  // Sync URL-based states after hydration
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);

    // Temporary chats from URL parameter
    if (urlParams.get("temporary-chat") === "true") {
      setTemporaryChatsEnabled(true);
    }

    // Team pricing dialog from URL hash
    if (window.location.hash === "#team-pricing-seat-selection") {
      setTeamPricingDialogOpen(true);
    }

    // Team welcome dialog from URL parameter
    if (urlParams.get("team-welcome") === "true") {
      setTeamWelcomeDialogOpen(true);
    }

    // PentestGPT migration confirm dialog from URL parameter
    if (urlParams.get("confirm-migrate-pentestgpt") === "true") {
  ...
  // Disabled in development mode when Convex is not configured
  const hasMigrationRun = useRef(false);
  const previousUserIdRef = useRef<string | null>(null);

  useEffect(() => {
    // Skip migration - Convex not configured in development mode
    ...
    // Migration would happen here if Convex was configured
    console.log("Skipping aggregate migration - Convex not configured");
  }, [user]);
