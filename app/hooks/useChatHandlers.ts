...
export const useChatHandlers = ({
  ...
  const handleRegenerate = async () => {
    ...
    // Remove the last assistant message from local state first
    // This is required for AI SDK's regenerate to work properly
    const lastAssistantIndex = lastAssistant
      ? messages.findIndex((m) => m.id === lastAssistant.id)
      : -1;
    const messagesWithoutLastAssistant =
      messages && messages.length > 0 && lastAssistantIndex !== -1
        ? messages.slice(0, lastAssistantIndex)
        : messages;

    // Update local messages state before calling regenerate
