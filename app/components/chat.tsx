...
import { useIsConvexAvailable } from "@/components/ConvexClientProvider";
...
export const Chat = ({
  ...
  // Use ref for selected model to avoid stale closures in auto-send
  ...
  // Skip all Convex queries when Convex is not configured
  const paginatedMessages = usePaginatedQuery(
    api.messages.getMessagesByChatId,
    isConvexAvailable && shouldFetchMessages ? { chatId } : "skip",
    { initialNumItems: 28 },
  );
  ...
  const chatData = useQuery(
    ...
    isConvexAvailable && shouldFetchMessages ? { id: chatId } : "skip",
  );
  ...
  // Only show "not found" if Convex is available and returned null (vs undefined when skipped)
