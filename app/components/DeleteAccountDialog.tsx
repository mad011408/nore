...
const useAuth = () => ({
  user: { id: "dev-user-001", email: "dev@hackerai.local", lastSignInAt: new Date().toISOString() },
  loading: false,
});
...
export const DeleteAccountDialog = ({
  ...
  const lastSignInAtIso: string | null = useMemo(() => {
    if (!user) return null;
    // User has lastSignInAt ISO string when available
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // 2) Cancel Stripe subs and delete user server-side
