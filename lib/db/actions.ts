...
export async function getMessagesByChatId({
  ...
  // If truncation returns empty but we have messages, just use the newest message
  // This can happen when Convex is not configured and file tokens can't be fetched
  if (!truncatedMessages || truncatedMessages.length === 0) {
    if (allMessages.length > 0) {
      // Fallback: return at least the newest message
      return {
        truncatedMessages: [allMessages[allMessages.length - 1]],
        chat,
        isNewChat
      };
    }

    // Only throw error if we truly have no messages
    throw new ChatSDKError(
      "bad_request:api",
      "No messages to process. Please try again.",
    );
  }
