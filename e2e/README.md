...
**Note:** Authentication is mocked in development mode. All test users are automatically available.
...
Authentication is mocked in development mode. Test users are automatically available.

To view available test users:
...
View test users (no creation needed with mock auth):
...
7. **Test real flows**: E2E tests use real Convex, not mocks
8. **Verify state**: Always verify both visual state (UI) and logical state (URLs, cookies)
9. **Compare titles**: When testing chat titles, compare sidebar and header titles for consistency

### Troubleshooting

**"Authentication failed" errors:**

- Authentication is mocked in development - this should not happen
- Verify `.env.e2e` has correct credentials if needed
